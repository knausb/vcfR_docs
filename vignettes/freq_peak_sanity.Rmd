...
ad <- extract.gt(vcf, element = 'AD')
#allele1 <- masplit(ad[,mySample, drop = FALSE], record = 1)
#allele2 <- masplit(ad[,mySample, drop = FALSE], record = 2)
allele1 <- masplit(ad, record = 1)
allele2 <- masplit(ad, record = 2)
...
myMat <- matrix(runif(9,0,1))
myMat[2,1] <- NA
myPeaks1 <- freq_peak(myMat, 1:nrow(myMat), winsize = winsize)
colnames(myMat) <- 'DDR7602'
rownames(myMat) <- paste("Var", 1:nrow(myMat), sep="")
myMat
...
myPeaks1 <- freq_peak(freq1[1:max_row, samp, drop=FALSE], getPOS(vcf)[1:max_row], winsize = winsize)



par(mfrow=c(2,1))

hist(freq1[, samp, drop=FALSE], breaks=seq(0,1,by=0.02), col="#C0C0C0")
abline(v=myPeaks1$peaks[4,1], col=2)


barplot(height=win[,4], width=0.02,  space = 0, horiz = F, add = FALSE, col="#A6CEE3")
axis(side=1)
abline(v=myPeaks1$peaks[4,1], col=2)
...
# Get tmp1 from compiled dput-like output.
