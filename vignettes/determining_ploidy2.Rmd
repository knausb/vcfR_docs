...
In theory, a heterozygous site should have each allele sequenced in a 50:50 ratio.
...
hist(tmp, breaks=seq(1,100,by=1), col="#808080")
...
hist(adp, breaks=seq(0, max(adp)+1, by=1), col="#C0C0C0", xlim = c(0,100))
...
adp <- adp[adp>0]
adp <- adp[adp<=100]
hist(adp, breaks=seq(0, max(adp), by=1), col="#C0C0C0")
#par(mfrow=c(1,1))
abline(v=sums[,"P17777us22"], col=2, lwd = 2)
```
...
sums <- apply(allele1, MARGIN=2, quantile, probs=c(0.20, 0.95), na.rm=TRUE)
sums
dp2 <- sweep(allele1, MARGIN=2, FUN = "-", sums[1,])
allele1[dp2 < 0] <- NA
dp2 <- sweep(allele1, MARGIN=2, FUN = "-", sums[2,])
allele1[dp2 > 0] <- NA
# Hard filter
#allele1[allele1 < 4] <- NA

is.na( vcf@gt[,-1][ is.na(allele1) ] ) <- TRUE
ad <- extract.gt(vcf, element = "AD")
