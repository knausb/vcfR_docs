...
vignette: >
  %\VignetteIndexEntry{Determining ploidy 2}
...
knitr::kable(ad[grep("[^0],[^0],[^0]", ad[,'P13626']),c(2:4,12)])
```
...
lw1 <- loess(y ~ x,data=data, span=0.75)
j <- order(data$x)
...
data <- data.frame(x = as.numeric(vcf@fix[,'POS']), y = ad2[,sample])
data <- data[!is.na(ad1[,sample]),]
lw2 <- loess(y ~ x,data=data, span=0.75)
j <- order(data$x)
lines(data$x[j],lw2$fitted[j],col="red",lwd=3)
...
## Estimation of allele balance



```{r}
sample <- "P17777us22"
bp1 <- hist(ad1[,sample], breaks = seq(0,1,by=0.02), plot = TRUE, col="#808080")
#bp2 <- hist(ad2[,sample], breaks = seq(0,1,by=0.02), plot = TRUE, add = TRUE, col="#C0C0C0")

d1 <- bp1$counts[ -1 ] - bp1$counts[ -length(bp1$counts) ]

segments(x0 = bp1$breaks[ -length(bp1$counts) ],
         y0 = bp1$counts[ -length(bp1$counts) ] - d1/2, 
         x1 = bp1$breaks[-1], 
         y1 = bp1$counts[ -length(bp1$counts) ] + d1/2,
         col = "#B22222", lwd = 3 )


( bp1$counts[ -1 ] - bp1$counts[ -length(bp1$counts) ] ) #/ 0.02


knitr::kable(cbind(bp1$mids[ -length(bp1$counts) ], bp1$counts[ -length(bp1$counts) ], d1))
...
The difference among allele balances can be a test for deviation from diploidy.
When both the major and minor allele are near 0.5 the difference should be near zero.
When the major and minor allele differ from 0.5 there should be a larger difference among the frequencies.
To test for a difference in ploidy among diploid and triploid a critical value of

$$
0.5 + (2/3-0.5)/2 = 0.583333
$$

Similarly, a critical value to test the difference among triploidy and tetraploidy could be calculates as

$$
2/3 + (0.75 - 2/3) / 2 = 0.7083333
$$
...
```{r}
sample <- "P17777us22"
bp1 <- hist(ad1[,sample], breaks = seq(0,1,by=0.02), plot = TRUE, col="#808080", xaxt='n')
axis(side=1, at=c(0,0.25,0.333,0.5,0.666,0.75,1), labels=c(0,"1/4","1/3","1/2","2/3","3/4",1))
abline(v=0.5, lwd=3, col="#228B22")
abline(v=2/3, lwd=3, col="#228B22")
abline(v=0.75, lwd=3, col="#228B22")

abline(v=0.583333, lwd=3, col="#B22222")
abline(v=0.708333, lwd=3, col="#B22222")
